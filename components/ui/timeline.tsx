...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <section
      className="h-auto w-full bg-primary drop-shadow-md dark:bg-neutral-950 md:px-10"
      ref={containerRef}
    >
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 lg:px-10">
        <h2 className="mb-4 max-w-4xl text-lg font-bold uppercase tracking-wide text-black dark:text-white md:text-4xl">
          Check my recent projects
        </h2>
        <p className="max-w-sm text-sm text-neutral-700 dark:text-neutral-300 md:text-base">
          Here&apos;s a timeline of my journey as a developer.
        </p>
      </div>
      ...
      <div ref={ref} className="relative mx-auto max-w-7xl">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex justify-start pt-10 md:gap-10 md:pt-4"
          >
